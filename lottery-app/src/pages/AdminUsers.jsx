...
export default function AdminUsers() {
  ...
  if (user?.role !== 'admin') {
    return (
      <div className="max-w-xl mx-auto bg-white p-8 rounded-2xl shadow-sm border border-[#dee9fd]">
        ...
        <form onSubmit={(e) => {
          ...
          <div>
            <label className="block text-sm font-bold text-[#434654] mb-2">ລະຫັດຜ່ານເກົ່າ</label>
            <input type="password" required
              className="w-full bg-[#eff3ff] border-none rounded-lg p-3"
              onChange={e => setPassData({...passData, current_password: e.target.value})}
            />
          </div>
          <div>
            <label className="block text-sm font-bold text-[#434654] mb-2">ລະຫັດຜ່ານໃໝ່</label>
            <input type="password" required
              className="w-full bg-[#eff3ff] border-none rounded-lg p-3"
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-white p-6 rounded-2xl shadow-sm border border-[#dee9fd]">
        <div>
          <h1 className="text-2xl font-black text-[#121c2a] mb-1">ຈັດການຜູ້ໃຊ້ (Users)</h1>
          <p className="text-sm text-[#737686]">ຈັດການສິດທິ, ພະນັກງານ ແລະ ລະຫັດຜ່ານ</p>
        </div>
        <button 
          onClick={() => {
            setEditingUser(null);
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-[#dee9fd] overflow-hidden">
        <table className="w-full text-left">
          <thead className="bg-[#eff3ff] text-[#434654] text-[11px] uppercase tracking-widest font-bold">
            <tr>
          ...
          <tbody className="text-sm divide-y divide-[#dee9fd]">
            {loading ? <tr><td colSpan="5" className="p-6 text-center text-[#737686]">Loading...</td></tr> : 
              users.map(u => (
              <tr key={u.user_id} className="hover:bg-[#f9f9ff] transition-colors">
                <td className="px-6 py-4 font-bold text-[#121c2a]">{u.username}</td>
                <td className="px-6 py-4 text-[#434654]">{u.full_name}</td>
                <td className="px-6 py-4">
                  <span className={`px-2 py-1 rounded text-xs font-bold ${u.role === 'admin' ? 'bg-[#ffdad6] text-[#ba1a1a]' : 'bg-[#e6eeff] text-[#003fb1]'}`}>
                    {u.role}
                  </span>
                </td>
                <td className="px-6 py-4">
                ...
                <td className="px-6 py-4 flex gap-2">
                  <button onClick={() => {
                    ...
                    setIsFormOpen(true);
                  }} className="text-[#003fb1] hover:underline font-bold text-xs bg-[#eff3ff] px-3 py-1.5 rounded-md">ແກ້ໄຂ</button>
                  <button onClick={() => {
                    setPassData({ target_user_id: u.user_id, new_password: '' });
                    setIsPassOpen(true);
                  }} className="text-[#00714d] hover:underline font-bold text-xs bg-[#eff3ff] px-3 py-1.5 rounded-md">Reset Pass</button>
                  {user.id !== u.user_id && (
          ...
          <div className="bg-white rounded-3xl w-full max-w-lg p-6 shadow-xl">
            ...
            <form onSubmit={handleSaveUser} className="space-y-4">
              {!editingUser && (
                <>
                  <div>
                    ...
                    <input required minLength={4} type="text" className="w-full bg-[#eff3ff] rounded-lg p-2.5" value={formData.username} onChange={e => setFormData({...formData, username: e.target.value.toLowerCase()})} />
                  </div>
                  <div>
                    <label className="block text-sm font-bold mb-1">Password</label>
                    <input required minLength={6} type="password" className="w-full bg-[#eff3ff] rounded-lg p-2.5" value={formData.password} onChange={e => setFormData({...formData, password: e.target.value})} />
                  </div>
                </>
              )}
              <div>
                <label className="block text-sm font-bold mb-1">ຊື່ເຕັມ (Full Name)</label>
                <input required type="text" className="w-full bg-[#eff3ff] rounded-lg p-2.5" value={formData.full_name} onChange={e => setFormData({...formData, full_name: e.target.value})} />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-bold mb-1">ຕຳແໜ່ງ (Role)</label>
                  <select className="w-full bg-[#eff3ff] rounded-lg p-2.5" value={formData.role} onChange={e => setFormData({...formData, role: e.target.value})}>
                ...
                {editingUser && (
                  <div>
                    ...
                    <select className="w-full bg-[#eff3ff] rounded-lg p-2.5" value={formData.is_active} onChange={e => setFormData({...formData, is_active: parseInt(e.target.value)})}>
              ...
              <div className="flex gap-2 pt-4">
                <button type="button" onClick={() => setIsFormOpen(false)} className="flex-1 py-2.5 bg-[#eff3ff] text-[#434654] font-bold rounded-xl hover:bg-[#dee9fd]">ຍົກເລີກ</button>
          ...
          <div className="bg-white rounded-3xl w-full max-w-sm p-6 shadow-xl">
            ...
            <form onSubmit={handleChangePassword} className="space-y-4">
              <div>
                ...
                <input required minLength={6} type="text" className="w-full bg-[#eff3ff] rounded-lg p-2.5" onChange={e => setPassData({...passData, new_password: e.target.value})} />
              </div>
              <div className="flex gap-2 pt-4">
                <button type="button" onClick={() => setIsPassOpen(false)} className="flex-1 py-2.5 bg-[#eff3ff] text-[#434654] font-bold rounded-xl hover:bg-[#dee9fd]">ຍົກເລີກ</button>
