...
export default function HotNumbers({ timeframe }) {
  ...
  return (
    <div className="md:col-span-8 bg-[#eff3ff] rounded-xl p-6 sm:p-8 flex flex-col justify-between">
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {hotNumbers.map(({ number, count }) => (
            <div
              key={number}
              className="bg-white p-5 sm:p-6 rounded-xl flex flex-col items-center gap-2"
            >
              <span className="text-4xl font-black text-[#006c49]">
                {number}
              </span>
              <span className="text-[10px] font-medium text-[#737686] uppercase tracking-widest text-center">
                ອອກ {count} ຄັ້ງ
              </span>
            </div>
          ))}
      ...
      <div className="mt-8 pt-6 border-t border-[#c3c5d7]/20 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
        <div className="space-y-1">
          <p className="text-sm text-[#737686]">ຄວາມເປັນໄປໄດ້ໃນງວດຖັດໄປ</p>
          <p className="text-2xl font-bold text-[#121c2a]">ສູງກວ່າຄ່າສະເລ່ຍ</p>
        </div>
