export default function AnimalCard({ icon, name, numbers, frequencyPercent, image_url }) {
  return (
    <div className="bg-white p-6 rounded-2xl flex flex-col items-center text-center">
      <div className="w-20 h-20 mb-4 flex items-center justify-center bg-[#e6eeff] rounded-full overflow-hidden relative">
        <img 
          src={image_url} 
          alt={`ນາມສັດ ${name}`}
      ...
      <h4 className="text-lg sm:text-xl font-bold mb-1 text-[#121c2a]">ນາມສັດ: {name}</h4>
      <p className="text-[#003fb1] text-sm font-semibold mb-4">ເລກ: {numbers}</p>
      <div className="w-full bg-[#dee9fd] rounded-full h-1.5 mb-2">
        <div
          className="bg-[#006c49] h-1.5 rounded-full transition-all duration-700"
          style={{ width: `${frequencyPercent}%` }}
        />
      </div>
      <span className="text-[10px] text-[#737686] uppercase tracking-wider">
