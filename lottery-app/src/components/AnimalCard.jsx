export default function AnimalCard({ icon, name, numbers, frequencyPercent, image_url }) {
  return (
    <div className="bg-card p-6 rounded-2xl flex flex-col items-center text-center">
      <div className="w-20 h-20 mb-4 flex items-center justify-center bg-secondary rounded-full overflow-hidden relative">
        <img 
          src={image_url} 
          alt={`ນາມສັດ ${name}`}
          className="w-full h-full object-cover z-10 bg-card"
          onError={(e) => {
            // Hide image and show material icon behind it if image fails to load
            e.target.style.opacity = '0';
          }}
        />
        <span className="material-symbols-outlined text-4xl text-[#003fb1] absolute inset-0 flex items-center justify-center z-0">
          {icon || 'pets'}
        </span>
      </div>
      <h4 className="text-lg sm:text-xl font-bold mb-1 text-foreground">ນາມສັດ: {name}</h4>
      <p className="text-[#003fb1] text-sm font-semibold mb-4">ເລກ: {numbers}</p>
      <div className="w-full bg-border rounded-full h-1.5 mb-2">
        <div
          className="bg-[#006c49] h-1.5 rounded-full transition-all duration-700"
          style={{ width: `${frequencyPercent}%` }}
        />
      </div>
      <span className="text-[10px] text-muted-foreground uppercase tracking-wider">
        ຄວາມຖີ່ {frequencyPercent}%
      </span>
    </div>
  )
}
