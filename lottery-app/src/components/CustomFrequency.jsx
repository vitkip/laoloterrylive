...
export default function CustomFrequency({ timeframe }) {
  ...
  return (
    <section className="bg-white rounded-3xl p-8 sm:p-10 border border-[#dee9fd] shadow-sm mb-16">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
        ...
        {/* Filter Selection */}
        <div className="flex bg-[#eff3ff] p-1.5 rounded-xl self-start">
          {[1, 3, 5, 'all'].map((y) => (
            <button
              key={y}
              ...
              className={`px-4 sm:px-6 py-2.5 text-sm font-bold rounded-lg transition-colors ${
                yearsFilter === y
                  ...
                  : 'text-[#737686] hover:text-[#121c2a] hover:bg-[#dee9fd]'
              }`}
            >
      ...
      {topNumbers.length > 0 ? (
        <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-8 lg:grid-cols-10 gap-4">
          {topNumbers.map((item, index) => (
            <div 
              key={item.number} 
              className="bg-[#f9f9ff] border border-[#dee9fd] rounded-xl p-4 flex flex-col items-center justify-center hover:shadow-md transition-shadow relative"
            >
              <div className="absolute top-2 left-2 text-[10px] font-black text-[#bac9e7]">#{index + 1}</div>
              <span className="text-2xl font-black text-[#121c2a] mb-1">{item.number}</span>
